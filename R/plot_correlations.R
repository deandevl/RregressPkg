...
#'
#' @description The data frame/matrix should have observations along the rows without NA values
...
#' @param create_plot A logical that if TRUE will create a heatmap plot of the correlations.
...
plot_correlations <- function(
  ...
  if(!anyNA(x)){
    ...
    if(create_plot){
      if(cluster_corr){
        hc <- stats::hclust(as.dist(1 - corr_mt))
