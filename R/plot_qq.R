...
#' @param plot_ref_line A logical which if TRUE will plot a reference line.
...
#' @param plot_ci_lines A logical which if TRUE will plot upper/lower confidence lines.
...
#' @param ci_linetype A string that sets confidence lines line type. Acceptable values are \code{twodash, solid,
#'  longdash, dotted, dotdash, dashed}
#' @param labels_n A numeric that sets the number of extreme values to label. If
#'  NULL then no points are labeled.
...
plot_qq <- function(
  ...
  if(plot_ci_lines){
    se_z <- function(z,n){
      return(sqrt(pnorm(z) * (1 - pnorm(z))/n)/dnorm(z))
    }
    ...
    a_plot <- a_plot +
      geom_line(data = ci_df, aes(x = ci_x, y = upper), color = ci_lines_color, size = ci_lines_width, linetype = ci_line_type) +
      geom_line(data = ci_df, aes(x = ci_x, y = lower), color = ci_lines_color, size = ci_lines_width, linetype = ci_line_type)
  }
  ...
  if(!is.null(labels_n)){
    ...
    a_plot <- a_plot +
      ggrepel::geom_text_repel(data = labels_dt, aes(label = id), color = label_color)
  }
