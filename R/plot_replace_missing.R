...
#'
#'  If the argument \code{replace_fun} is \code{NULL} then only a bar chart showing the missing value count for each variable is
#'  returned.
#'
...
#'  replacement value. See an example in the \code{demo} folder.
...
#'  \item "replacement_df" -- a data.table copy of \code{df} with missing values replaced
